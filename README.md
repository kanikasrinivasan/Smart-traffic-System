...
### Run AI detector only (YOLO + OpenCV)

```bash
python main.py --source night_traffic.mp4 --weights yolov8n.pt --show
```

### Run simulation in mock mode (works without model/video)

```bash
python simulation.py
```

### Run simulation with real detector subprocess

```bash
python simulation.py --detector-cmd "python main.py --source night_traffic.mp4 --weights yolov8n.pt"
```

> Press `q` in the OpenCV window to stop detection when using `--show`.
